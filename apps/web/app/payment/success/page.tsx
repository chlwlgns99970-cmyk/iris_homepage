...
import { getCurrentAuth, getPaymentOrder, type PaymentOrder } from '@/lib/api';

export default function PaymentSuccessPage() {
  const [order, setOrder] = useState<PaymentOrder | null>(null);
  const [message, setMessage] = useState('지급 상태를 확인하는 중입니다.');
  ...
  useEffect(() => {
    ...
    getCurrentAuth().then(async (auth) => {
      ...
      const result = await getPaymentOrder(orderId);
  ...
  return <PaymentResultShell eyebrow="PAYMENT RESULT" title={order?.status === 'completed' ? '결제가 완료되었습니다.' : '결제 상태를 확인해 주세요.'}>
    ...
    {order && <dl className="payment-result-list">
      ...
      <div><dt>지급 상태</dt><dd>{order.status === 'completed' ? '지급 완료' : '처리 상태 확인 필요'}</dd></div>
